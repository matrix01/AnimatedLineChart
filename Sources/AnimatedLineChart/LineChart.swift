...
class LineChart: UIView {
    ...
    var minimumLabelWidth: CGFloat = 60.0
    ...
    override func layoutSubviews() {
        ...
        drawFirstLastLabels(for: chartItems)
//        drawLabels(for: chartItems)
        ...
        guard let lastItem = dataPoints.last else {
            return
        }
        
        showDotOnLastItem(for: lastItem)
    }
    ...
    private func convertChartItemsToPoints(_ items: [ChartItem]) -> [CGPoint]? {
        ...
        var max = 0
        var min = calculateMinValue(for: items) ?? 0
    ...
    */
    ...
    /**
     Create titles at the bottom for only first and last entry
     */
    private func drawFirstLastLabels(for dataEntries: [ChartItem]) {
        guard dataEntries.count > 1, lineGap > 0 else {
            return
        }

        let entries = [dataEntries.first, dataEntries.last].compactMap{ $0 }
        let verticalGridValues: [CGFloat] = [0, 1]
        
        for (index, value) in verticalGridValues.enumerated() {
            let labelWidth = dataEntries.count > 1 ? minimumLabelWidth : lineGap
            let y = gridLayer.frame.size.height + 4
            let x = value * gridLayer.frame.width - (value * labelWidth)
            ...
            textLayer.string = entries[index].label
            textLayer.frame = CGRect(x: x, y: y, width: labelWidth, height: 16)
    ...
    /**
     Create vertical lines (grid lines) and show the value of each line
     */
    private func addVerticalLine(from: CGPoint, to: CGPoint) {
        let verticalLinePath = UIBezierPath()
        verticalLinePath.move(to: from)
        verticalLinePath.addLine(to: to)
    ...
    private func drawHorizontalLines(with dataEntries: [ChartItem]) {
        ...
        if let gridValues = gridValues {
            ...
            for value in verticalGridValues {
                let width = value * gridLayer.frame.width
                let height = gridLayer.frame.height
                let firstPoint = CGPoint(x: width, y: 0)
                let endPoint = CGPoint(x: width, y: height)
                addVerticalLine(from: firstPoint, to: endPoint)
            }
            ...
            for value in gridValues {
                ...
                let textLayer = CATextLayer()
                textLayer.frame = CGRect(x: gridLayer.frame.size.width + 3, y: height - 10, width: 50, height: 16)
                textLayer.foregroundColor = axisLabelsColor.cgColor
                textLayer.backgroundColor = UIColor.clear.cgColor
                textLayer.contentsScale = UIScreen.main.scale
                textLayer.fontSize = 11
    ...
    /**
     Create a the indicator on last item of chart
     */
    private func showDotOnLastItem(for dataPoint: CGPoint) {
        indicatorLayer.isHidden = true
        indicatorLayer.removeFromSuperlayer()
        let xValue = (dataPoint.x - 16) + leftSpace
        let yValue = (dataPoint.y - 16) + topSpace
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        
        indicatorLayer.frame = CGRect(x: xValue, y: yValue, width: 16, height: 16)
    ...
    private func handleTouchEvents(_ touches: Set<UITouch>) {
        ...
        showDotOnLastItem(for: closestPoint)
    }
    ...
    */
    ...
    Calculate the top value, rounded to a multiple of 100
    */
    private func calculateMinValue(for dataEntries: [ChartItem]) -> Int? {
        guard let maxValue = dataEntries.min()?.value else {
            return nil
        }
